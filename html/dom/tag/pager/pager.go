package pager

import (
	"bytes"
	"fmt"
	"strconv"
	"strings"
)

const (
	FirstLinkFormat = ""
	PagerLinkFormat = "?page=%d"
	PagerLinkCount  = 7

	NextPageText     = "›"
	PreviousPageText = "‹"
	// FirstPageText    = PreviousPageText
	// LastPageText     = NextPageText
	FirstPageText   = "«"
	LastPageText    = "»"
	DefaultPageSize = 10
)
const (
	CONTROL = 1 << iota
	PREVIOUS
	NEXT
)

var (
	pagerGetter PagerGetter = new(defaultPagerGetter)
)

// 分页产生器
type PagerGetter interface {
	SetPager(*UrlPager) error
	//page为当前页
	Get(page, flag int) (url, text string)
}

// 默认的产生器，在查询后添加page=?
type defaultPagerGetter struct {
	p *UrlPager
}

func (d *defaultPagerGetter) SetPager(v *UrlPager) error {
	d.p = v
	return nil
}

func (d *defaultPagerGetter) Get(page, flag int) (url, text string) {
	if flag&CONTROL != 0 {
		if flag&PREVIOUS != 0 {
			if page == 1 {
				return "javascript:;", FirstPageText
			}
			return fmt.Sprintf(d.p.Query, page-1), PreviousPageText
		}

		if flag&NEXT != 0 {
			if page == d.p.Pages {
				return "javascript:;", LastPageText
			}
			return fmt.Sprintf(d.p.Query, page+1), NextPageText
		}
	}

	if -page == 1 && len(FirstLinkFormat) != 0 {
		return fmt.Sprintf(d.p.Query, 1), "1"
	}
	return fmt.Sprintf(d.p.Query, page), strconv.Itoa(page)
}

type UrlPager struct {
	//当前页面索引,从0开始
	Index int
	//查询条件
	Query string
	//连接和页码
	getter PagerGetter
	//页面总数
	Pages int
	//链接长度,创建多少个跳页链接
	Number int
	//记录条数
	Total int
	//页码文本格式
	pageTextFormat string
	//是否允许输入页码调页
	enableInput bool
	//使用选页
	enableSelect bool
	//分页详细记录,如果为空字符则用默认,为空则不显示
	PagerTotal string
	// 当总页数为1时，是否显示分页
	PagingOnZero bool
	// 是否显示分页的汇总信息
	ShowSummary bool
	// 分页大小
	PageSize int
}

func (u *UrlPager) check() {
	if u.Index < 1 {
		u.Index = 1
	}
	if len(strings.TrimSpace(u.Query)) == 0 {
		u.Query = PagerLinkFormat
	}
}

func (u *UrlPager) Pager() []byte {
	var bys *bytes.Buffer
	var cls string
	var url, text string

	//检查数据
	u.check()
	u.getter.SetPager(u)

	//开始拼接html
	bys = bytes.NewBufferString(`<div class="fixed-table-pagination">`)

	// 摘要信息
	if u.ShowSummary {
		//显示信息
		const pagerStr = ` <div class="pull-left hidden-xs hidden-sm">
		                    <span class="pagination-info">第 %d/%d 页，总共 %d 条记录</span>
							<span class="">每页显示 %d 条记录</span>
		                </div>`
		// const pagerStr string = "<span class=\"info\">&nbsp;第%d/%d页，共%d条。</span>"
		if len(u.PagerTotal) == 0 {
			u.PagerTotal = pagerStr
		}
		bys.WriteString(fmt.Sprintf(u.PagerTotal, u.Index, u.Pages, u.Total, u.PageSize))
	}

	// pager items
	bys.WriteString(`<div class="pull-right pagination"><ul class="pagination"><ul class="pagination">`)
	//输出上一页
	if u.Index > 0 {
		cls = "page-pre"
		url, text = u.getter.Get(u.Index, CONTROL|PREVIOUS)
	} else {
		cls = "page-next"
		url, text = u.getter.Get(u.Index, CONTROL|PREVIOUS)
	}
	bys.WriteString(fmt.Sprintf(`<li class="%s"><a href="%s">%s</a></li>`, cls, url, text))

	linkNumber := u.Number //链接接数(默认10)
	currIndex := u.Index   //当前页数
	pageCount := u.Pages   //总页数
	beginPage := currIndex //开始页

	//计算开始页,将自动补全左右的链接
	offset := linkNumber/2 + linkNumber%2
	//判断是否为最后组,且不为第一组
	if beginPage-offset > pageCount-linkNumber && pageCount-linkNumber > 0 {
		beginPage = pageCount - linkNumber
	} else if beginPage > offset && beginPage != pageCount { //超出第一组,但不为最后一组
		beginPage = beginPage - offset
	} else {
		beginPage = 0
	}
	//拼接页码
	for i, j := 1, beginPage; i <= linkNumber && j < pageCount; i++ {
		j++
		if j == currIndex {
			//如果为页码为当前页
			bys.WriteString(fmt.Sprintf(`<li class="active"><a>%d</a></li>`, j))
		} else {
			//页码不为当前页，则输出页码
			u, t := u.getter.Get(j, 0)
			bys.WriteString(fmt.Sprintf(`<li><a href="%s">%s</a></li>`, u, t))
		}
	}

	//输出下一页链接
	if u.Index < u.Pages {
		cls = "page-next"
		url, text = u.getter.Get(u.Index, CONTROL|NEXT)
	} else {
		cls = "page-disabled"
		url, text = u.getter.Get(u.Index, CONTROL|NEXT)
	}
	bys.WriteString(fmt.Sprintf(`<li class="%s"><a href="%s">%s</a></li>`, cls, url, text))

	bys.WriteString("</ul></div>")
	bys.WriteString("</div>")
	return bys.Bytes()
}

func (u *UrlPager) PagerString() string {
	if !u.PagingOnZero && u.Pages == 1 {
		return ""
	}
	return string(u.Pager())
}

// func NewUrlPager(pages int, current int, query string) *UrlPager {
// 	p := &UrlPager{}
// 	p.Pages = pages
// 	p.Index = current
// 	p.Number = PagerLinkCount
// 	p.Query = query
// 	p.ShowSummary = true
// 	p.getter = pagerGetter
// 	p.getter.SetPager(p)
// 	return p
// }
func NewUrlPager(query string, total int, current int, pageSize ...int) *UrlPager {
	p := &UrlPager{}
	p.Total = total
	p.PageSize = DefaultPageSize
	if len(pageSize) > 0 {
		p.PageSize = pageSize[0]
	}
	p.Pages = MathPages(p.Total, p.PageSize)
	p.Index = current
	p.Number = PagerLinkCount
	p.Query = query
	p.ShowSummary = true
	p.getter = pagerGetter
	p.getter.SetPager(p)
	return p
}

// 获取总页数
func MathPages(total, size int) int {
	p := total / size
	if total%size == 0 {
		return p
	}
	return p + 1
}
